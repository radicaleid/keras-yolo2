...
class BatchGenerator(Sequence):
  ...
  def __len__(self):
    return int(np.ceil(float(len(self.filelist))*self.config['EVTS_PER_FILE']/self.config['BATCH_SIZE']))   
  ...
  def load_annotation(self, i):
    annots = []
    ifile=int(np.ceil(i/self.config['EVTS_PER_FILE']))
    ievent=i-ifile*self.config['EVTS_PER_FILE']
  ...
  def load_image(self, i):
    ...
    return imagins['raw'][ievent]
  
  def __getitem__(self, idx):
    start = time.time()
    im_batch = np.zeros((self.config['BATCH_SIZE'], self.config['IMAGE_C'], self.config['IMAGE_H'], self.config['IMAGE_W']))                         # input images
    box_batch = np.zeros((self.config['BATCH_SIZE'], 1     , 1     , 1    ,  self.config['TRUE_BOX_BUFFER'], 4))   # list of self.config['TRUE_self.config['BOX']_BUFFER'] GT boxes
    y_batch = np.zeros((self.config['BATCH_SIZE'], self.config['GRID_H'],  self.config['GRID_W'], self.config['BOX'], 4+1+len(self.config['LABELS'])))                # desired network output


    instance_count=0
    for train_instance in xrange(idx*self.config['BATCH_SIZE'],(idx+1)*self.config['BATCH_SIZE']):
      
      ifile=int(np.ceil(train_instance/self.config['EVTS_PER_FILE']))
      ievent=train_instance-ifile*self.config['EVTS_PER_FILE']
      #print 'get event ',train_instance, ' from' ,ievent,' of file', ifile
      # augment input image and fix object's position and size
      #img, all_objs = self.aug_image(train_instance, jitter=self.jitter)
      imagin_files=np.load(self.filelist[ifile])
      imagins=imagin_files['raw'][ievent]
      all_objs=imagin_files['truth'][ievent]
      ...
      for obj in all_objs:
        ...
        if grid_x < self.config['GRID_W'] and grid_y < self.config['GRID_H']:
          ...
          #print instance_count, grid_y, grid_x, best_anchor, y_batch[instance_count, grid_y, grid_x, best_anchor]
          #print 'true box', y_batch[instance_count, grid_y, grid_x, best_anchor, 0:4]
        ...
        if self.norm != None: 
            im_batch[instance_count] = self.norm(imagins)
        # else:
        ...
        else: im_batch[instance_count] = self.norm(imagins)
        instance_count+=1
    
    #print(' new batch created', idx)
    #print 'im_batch, box_batch, y_batch: ', im_batch.shape, box_batch.shape, y_batch.shape
    #print box_batch
    print('idx ',idx*self.config['BATCH_SIZE'],(idx+1)*self.config['BATCH_SIZE'],':',time.time() - start)
